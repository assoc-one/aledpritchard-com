...
// slide is the `fit` or `fill` variant, so the slide image reads as bounded to
// the right column rather than full-bleed. (`fill` is an alias of `fit` until
// the true contain behavior lands in T4/COS-200.)
export function FitPanel({ projects }: { projects: Project[] }) {
  ...
  return (
    <div
      ...
      style={{ opacity: variant === "fit" || variant === "fill" ? 1 : 0 }}
    />
  );
}
